...
def test_build_workbook_creates_all_expected_tabs(tmp_path):
    ...
    build_workbook(kb, cv_versions, cover_letters, output)
...
def test_build_workbook_lists_become_comma_joined_strings(tmp_path):
    kb = StructuredKnowledgeBase(
        experience=[
            ExperienceBullet(
...
def test_build_workbook_handles_empty_lists_without_error(tmp_path):
    output = tmp_path / "empty.xlsx"
    build_workbook(StructuredKnowledgeBase(), [], [], output)
    wb = openpyxl.load_workbook(output)
