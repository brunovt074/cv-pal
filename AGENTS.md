...
profiles), job-posting-by-URL (needs a `WebContentPort` adapter), HTTP/SSE MCP transport
(deliberately out of scope - see Decision Log).
...
| Multi-user is a parameter, not implemented - Bruno is the test fixture | The user's explicit direction: design every new piece to receive the knowledge base/paths as data, never hardcode "Bruno". Known remaining seams, not to be deepened further: `markdown_knowledge_repository.py`'s header has Bruno's name hardcoded; `personal_data_resolution.py`'s `PREFERRED_VALUES` are inherently per-user; `config.Settings` paths aren't yet namespaced per user. None of this session's new code adds a new Bruno-specific assumption |
