...
`src/cvpal/application/prompts/voice.py` (the extraction prompt) — extracting a reusable "how
Bruno writes" style guide from his existing cover letters, so tailored CVs and cover letters
sound like him instead of generic AI output. This is not a separate pipeline step — it runs
...
- **Source of truth is the existing cover letters** (`RawDocument` records with
  `doc_kind="cover_letter"`, ingested alongside the CVs). Do not ask Bruno to write a new sample
...
- **Output is the `Voice Profile` section of `data/knowledge-base.md`** — human-readable (a
  Trait/Value table Bruno can review/correct directly) and machine-readable (parsed back into
...
- **Language-aware.** `language_specific_traits` should note whether the observed patterns hold
  ...
  between them (common — people write more formally in a second/professional language). His
  English is B2 — `cv-generation` needs to reflect technical competence without pretending native
  fluency, not smooth it over.
...
- Since this is fundamentally a qualitative LLM task, testing is: (1) the plumbing
  ...
  verification step — generate a paragraph using the profile and confirm it sounds like Bruno.
  Don't try to unit-test "does this sound like Bruno" programmatically.
