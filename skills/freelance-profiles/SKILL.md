...
bio, skills list, suggested rates, portfolio blurb) for freelance platforms, for the author to
paste and review manually.
...
well-crafted **content**; the author performs the actual paste/submit action themselves.
...
- **Draw from the same knowledge base and voice profile as `cv-generation`** — don't re-derive
  the author's background or tone independently. A freelance profile is still the author's voice,
  adapted to a different format and audience, not a different persona.
- **Output as markdown per platform** in `data/outputs/profiles/{platform}.md`, structured so
  each field the author needs to paste is clearly labeled (Headline:, Bio:, Skills:, ...) —
  optimize for "easy to copy one field at a time into a web form", not for prose readability.

## Testing

- Verification is manual: generate the four platform drafts, have the author read and confirm
  before they paste anything live. No automated test can validate "does this read well on
  Upwork" — don't try.
