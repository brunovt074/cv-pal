"""Deterministic (no LLM) inventory of source CV documents.

Populates the CVVersions tab: one row per source file, grouped by "stack"
(the top-level folder under the CV root, e.g. Java/, PHP/, generic/), with
the most-recently-modified file in each stack flagged as the current variant.
"""
...
from cvpal.analytics.models import CVVersionEntry
